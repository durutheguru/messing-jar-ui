...
export default class Util {
  ...
  public static searchImageUrl(arr: any[]) {
    ...
    return arr.length && obj && obj.publicUrl
      ? obj.publicUrl
      : defaultBackground;
  }
  ...
  public static deepGet(parent: any, path: string) {
    ...
    for (let k = 0, kLen = paths.length; k < kLen; k++) {
      if (
        parent != null &&
        Object.prototype.hasOwnProperty.call(parent, paths[k])
      ) {
  ...
  public static extractError(errorResponse: any) {
    ...
    return Util.isValidString(errorMsg, false)
      ? Util.errorSanitize(errorMsg)
      : "Unknown Error";
  }
  ...
  public static extractGqlError(error: ApolloError): string {
    ...
    return Util.isValidString(msg) &&
      msg.indexOf(Constants.gqlDefaultErrorDelimeter) > 0
      ? msg.substring(
          msg.indexOf(Constants.gqlDefaultErrorDelimeter) +
            Constants.gqlDefaultErrorDelimeter.length
        )
      : msg;
  }
  ...
  public static copyArray(src: any, dest: any) {
    if (!Util.isValidArray(src, true) || !Util.isValidArray(dest, true)) {
      throw new Error(
        "Both source and Destination must be valid Javascript arrays"
      );
    }
  ...
  public static zoneDateTimeMoment(
    zoneDateTime: string,
    format?: string
  ): string {
    return Util.moment(
  ...
  public static stringifyZonedDateTime(date: string, time: string): string {
